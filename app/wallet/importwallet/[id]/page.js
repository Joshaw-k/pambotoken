...
export default function page() {
  ...
  const HandleWallet = () => {
    ...
    tabs.forEach((tab, index) => {
      if (tab.id == tabs[0].id) {
        tab.classList.add("tabActive", "bg-[#FFED76]", "text-white");
        tab.classList.remove("text-[#5B5B5B]");
      }

      tab.addEventListener("click", (e) => {
        tabs.forEach((tab) => {
          tab.classList.remove("tabActive", "bg-[#FFED76]", "text-white");
                ...
                <div>
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex font-urbanist justify-between items-center mt-5">
                      ...
                      <button
                        className="bg-[#FFED76] py-5 w-[48%] rounded-full text-[#222222] font-bold"
                        type="submit"
